...
class AcquisitionTask(models.Model):
    ...
    ACQUISITION_TYPES = [
        ...
        ("file_upload", "Evidence File Upload"),
    ]

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    evidence = models.OneToOneField(
        Evidence, on_delete=models.CASCADE, related_name="acquisition_task", null=True
    )
    ...
    source_upload = models.FileField(
        upload_to="acquisition_uploads/%Y/%m/%d/", blank=True, null=True
    )
    destination_path = models.CharField(max_length=1024, blank=True, null=True)
